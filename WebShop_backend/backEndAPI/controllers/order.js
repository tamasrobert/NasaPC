...
// custom price converter ---> 16900 -> 16.900
...
exports.placeOrder = (req, res) => {
    ...
    if(!session) {
        Order.insertMany({id, 'created_at': Date.now(), 'status': 'Feldolgozás alatt...', ...req.body}).then(() => {
            console.log('order saved for guest');
        ...
        .then((user) => {
            ...
            Order.insertMany({id, 'userId': new mongodb.ObjectId(user._id), 'created_at': Date.now(), 'status': 'Feldolgozás alatt...', ...req.body}).then(() => {
                console.log('order saved for registered user');
