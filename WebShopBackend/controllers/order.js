                ...
                Order.insertMany({ id, 'userId': new mongodb.ObjectId(user._id), 'created_at': Date.now(), 'status': 'Processing...', ...req.body })
                    .then(() => {
                        ...
                        imagePaths.forEach(imagePath => {
                            files.push({
                                'filename': imagePath,
                                'content': base64_encode('../WebShop/src/assets/image/' + imagePath),
                                'type': 'image/jpg',
                                'cid': 'imagecid' + index,
                                'disposition': 'inline',
                            });
